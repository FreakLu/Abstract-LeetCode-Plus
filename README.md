...
1. Backend Service
The backend ships with a free default LLM endpoint, so `.env` is optional for quick installs. For custom API providers, see the Backend Sub-README.
...
1. 后端服务启动
后端默认使用免费的 LLM 接口，快速安装不需要先配置 `.env`。如需切换自定义 API 或供应商，请参考后端子目录说明。
